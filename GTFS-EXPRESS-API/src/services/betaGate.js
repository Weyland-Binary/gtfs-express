 ...
 *                              tier?, quota? } }
 *     - `tier`  (string, optional) — "beta" (default) | "unlimited" | …
 *     - `quota` (object, optional) — per-code overrides, e.g.
 *       { dailyAi: 5000, hourlyAi: 400 }. Absent → the tier defaults apply.
...
const validateCode = (rawCode) => {
  ...
  return {
    ...
    code: normalized,
    // Optional per-code tier + quota overrides (used to grant a comfortable
    // "quasi-unlimited" beta experience). Both are additive and default to a
    // plain "beta" tier with no per-code overrides, so existing codes.json
    // entries (a bare { email, label } object) keep working unchanged.
    tier: typeof entry.tier === "string" ? entry.tier : "beta",
    quota:
      entry.quota && typeof entry.quota === "object" && !Array.isArray(entry.quota)
        ? entry.quota
        : null,
  };
};
