 ...
 * @param {number} [opts.dailyLimit]  — per-code 24h cap override (beta tier /
 *        per-code quota). Falls back to NL2SQL_DAILY_LIMIT_PER_CODE.
 * @param {number} [opts.hourlyLimit] — per-code 1h cap override. Falls back to
 *        the scope's configured hourly cap.
 *        NB: the GLOBAL budget is never overridable — it always reads from
 *        config so it stays the operator-wide kill-switch for every key.
 ...
 */
const check = ({ key, scope, dailyLimit, hourlyLimit } = {}) => {
  const k = key || "anon";
  ...
  // Per-code caps may be raised for beta-code holders via explicit overrides
  // (resolved by the caller from the code's tier / quota). The GLOBAL budget
  // is intentionally NOT overridable — it stays the operator-wide kill-switch.
  const hourlyCap =
    hourlyLimit != null ? _resolveLimit(hourlyLimit) : _hourlyLimitFor(s);
  const dailyCap =
    dailyLimit != null
      ? _resolveLimit(dailyLimit)
      : _resolveLimit(config.NL2SQL_DAILY_LIMIT_PER_CODE);
  ...
  const snapshot = () => ({
    hourly: { used: hourly.length, limit: hourlyCap },
    daily: { used: daily.length, limit: dailyCap },
...
/**
 * Resolve the per-code AI caps for a beta-code holder, ready to spread into
 * check(). Returns {} for keyless / anonymous callers so check() keeps the
 * strict config defaults. A code's own `quota.dailyAi` / `quota.hourlyAi`
 * (from codes.json) overrides the beta tier default.
 *
 * @param {?{code?: string, quota?: {dailyAi?: number, hourlyAi?: number}}} betaTester
 *        the validated beta context (req.betaTester), or null/undefined.
 */
const betaLimitsFor = (betaTester) => {
  if (!betaTester || !betaTester.code) return {};
  const quota = betaTester.quota || {};
  const dailyLimit =
    Number.isFinite(quota.dailyAi) && quota.dailyAi > 0
      ? quota.dailyAi
      : config.BETA_NL2SQL_DAILY_LIMIT_PER_CODE;
  const hourlyLimit =
    Number.isFinite(quota.hourlyAi) && quota.hourlyAi > 0
      ? quota.hourlyAi
      : config.BETA_NL2SQL_RATE_LIMIT;
...
module.exports = { check, betaLimitsFor, snapshot, _reset };
