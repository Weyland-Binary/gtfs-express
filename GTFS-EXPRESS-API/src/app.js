...
// 🚦 Strict rate limiting for uploads: configurable via .env
...
const revalidateLimiter = rateLimit({
  ...
  max: 5,
  keyGenerator: (req) =>
    req.headers["x-session-id"] || ipKeyGenerator(req.ip) || "unknown",
  message: "Revalidation limit of 5/minute reached. Please wait before re-running.",
  standardHeaders: true,
  legacyHeaders: false,
});
...
// 🚦 SQL console rate limiting: 60 req/min keyed on X-Session-ID
const sqlLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: parseInt(process.env.RATE_LIMIT_MAX_SQL || "60"),
  keyGenerator: (req) =>
    req.headers["x-session-id"] || ipKeyGenerator(req.ip) || "unknown",
