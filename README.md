...
- **Network Studio (design from scratch)**: a brief or a territory becomes a network spec (stops, lines, services, holidays), routed on roads, scored (coverage, service level, fleet, cost, accessibility, conformance to the brief) and compiled into a validated GTFS.
- **Change Studio (change an existing feed)**: a service-change brief becomes a typed, sourced change plan — about forty deterministic operations (frequencies, spans, trips, extensions, cut-backs, works detours, stops, calendars and holidays, lines, running times, connections, fares, vehicle blocks, row-level edits) scoped by dates, periods, school days or public holidays. Every plan is previewed on a copy: what really changes per line and period, integrity, the validator's new errors, consumer checks (Google-style), impact (km, hours, cost, fewest vehicles), network quality before → after, conformance to the brief — then applied as one undoable edit. Never guesses: a missing or ambiguous parameter becomes a question.
- **Passenger information & interop**: GTFS-RT service alerts and a notice generated from what a plan really changes; the change as a GTFS Diff; other operators' timetables (e.g. regional trains) loaded as read-only references to align connections on.
